...
namespace App\Http\Controllers\admin\admin_home\staff;
...
class AlluserApplicationController extends Controller
{
    //
    ...
    public function index(Request $request){
        ...
        if($search !=""){
          ...
          $all=leave_application::where('status',1)->orderby('application_id','desc')->paginate(7);
        }
          
          //dd($creatorData);
          return view('admin.staff.LeaveApplication.index',compact('all','search'));
      }
      public function add(){
        ...
        //$years = range(date("Y"), date("Y") - 25);
          return view('admin.staff.LeaveApplication.add',compact('leavetype','leavereson'));
      }
      public function edit($slug){
        ...
        $alldata=leave_application::where('status',1)->where('slug',$slug)->firstOrFail();
          ...
          return view('admin.staff.LeaveApplication.edit',compact('alldata','leavetype','leavereson'));
        }
        public function view($slug){
          ...
          //dd($prodata);
          $alldata=leave_application::where('status',1)->where('slug',$slug)->firstOrFail();
          return view('admin.staff.LeaveApplication.view',compact('alldata','prodata'));
        }
        ...
        public function insert(Request $request){
          ...
          if ($insert) {
            ...
            return redirect('admin/dashboard/manage/leave_application/add')->with('message',' Information Added successful !');
        } else {
            Session::flash('error','value');
            return redirect('admin/dashboard/manage/leave_application/add')->with('error',' Information Added Failed 1');
        }
        }
        public function update(Request $request){
          $startDate = $request['ap_s_date'];
          $endDate = $request['ap_e_date'];
          $startDateObj = new DateTime($startDate);
          $endDateObj = new DateTime($endDate);
          $interval = $startDateObj->diff($endDateObj);
          $totaldays = $interval->days;
            // date sum 
          $aproved = Auth::guard('admin')->user()->name;
          $id=$request['id'];
          $user_id=$request['user_id'];
          $slug=$request['slug'];
          $update=leave_application::where('status',1)->where('application_id',$id)->update([
            'dpt_comment'=>$request['dpt_comment'],
            'ap_s_date'=>$request['ap_s_date'],
            'ap_e_date'=>$request['ap_e_date'],
            'note'=>$request['note'],
            'instraction'=>$request['instraction'],
            'aproved_days'=>$totaldays,
            'uniqe_id'=>$aproved,
            'updated_at'=>Carbon::now()->toDateTimeString(),
          ]);
          if($update){
            Session::flash('success','value');
            return redirect('admin/dashboard/manage/leave_application/view/'.$slug);
          }else{
            Session::flash('error','value');
            return redirect('admin/dashboard/manage/leave_application/edit/'.$slug)->with('message','Information Update Failed !');
          }
        }
    ...
    // permanent delete data from Database and image into the public folder directory 
      ...
      if ($data) {
        ...
        public function recycle(){
         ...
         // dd($all);
          return view('admin.staff.LeaveApplication.recycle',compact('all'));
        }
}
